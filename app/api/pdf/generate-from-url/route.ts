...
import { GeneratePdfFromUrlSchema } from '@/lib/utils/validation'
...
export async function POST(req: NextRequest) {
  try {
    ...
    const bodyValidation = GeneratePdfFromUrlSchema.safeParse(body)
    ...
    const { url, format, filename, wait_for, full_page } = bodyValidation.data

    // Generate PDF from URL
    const pdfResult = await generatePdfFromUrl(url, {
      format,
      filename,
      waitFor: wait_for,
      fullPage: full_page,
    ...
    await logApiUsage({
      apiKeyId: apiKeyId!,
      userId: userId!,
      endpoint: '/api/pdf/generate-from-url',
      method: 'POST',
